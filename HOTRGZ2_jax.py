...
def RepMat(dimV1R1,dimV1R2,dimV2R1,dimV2R2):
    ...
    P=np.zeros([dimV1*dimV2,dimV1,dimV2])
    counter=0
...
def is_isometry(g):
    return np.isclose(g@g.T.conj(),np.eye(g.shape[0])).all()
...
class HOTRGLayer:
    ...
    gg:'list[list[np.ndarray]]'=None
    hh:'list[list[np.ndarray]]'=None
    _gg1=None
    ...
    def make_gg_isometric(self):
        ...
        for i in range(len(self.gg)):
            for j in range(0,len(self.gg[i]),2):
                ...
                assert np.isclose(self.gg[i][j]@self.gg[i][j+1].T.conj(),np.eye(self.gg[i][j].shape[0])).all()
    ...
    def get_insertion(self):
        if self.gg:
            ...
            return np.eye(self.tensor_shape[0])
...
def forward_layer(Ta,Tb,layer:HOTRGLayer,use_checkpoint=False)->np.ndarray:
...
def forward_observalbe_tensor_moments(T0_moments:'list[np.ndarray]',layers:'list[HOTRGLayer]',
    ...
    for ilayer,layer in tqdm(list(enumerate(layers)),leave=False):
        ...
        Tms1=[np.zeros_like(Tms[0])]*len(Tms)
