...
parser.add_argument("--isomcorr", dest = "isomcorr", action="store_true",
                    help = "whether to correct the isometry in the response matrix")
...
for k in range(lowRG, hiRG):
    ...
    RABs = RABslist[k]
    RABsh = RABshlist[k]
    scDims = diffGiltHOTRG(Acur, Anormcur, isom, RABs, RABsh, scaleN = NscaleD,isom_corr=isom_corr)
    endT = time.time()
    diffT = endT - startT
    # if type(scDims) == int:
